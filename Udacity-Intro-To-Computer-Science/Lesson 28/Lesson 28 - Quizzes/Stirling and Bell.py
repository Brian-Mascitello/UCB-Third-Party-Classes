...
def stirling(num_items, num_sets):
    # S(n, k) = k*S(n-1, k) + S(n-1, k-1)
    if num_items < num_sets:
        return 0
    elif num_items == 1:
        return 1
    elif num_sets == 1:
        return 1
    elif num_items == num_sets:
        return 1
    else:
        return num_sets * stirling(num_items - 1, num_sets) + stirling(num_items - 1, num_sets - 1)


def bell(integer_input):
    # B(n) is the sum of S(n,k) for k = 1, 2, ... , n
    summation = 0
    num_sets = 1
    while integer_input >= num_sets:
        summation += stirling(integer_input, num_sets)
        num_sets += 1
    return summation
