...
def ordered_search(index, ranks, keyword):
    look = lookup(index, keyword)
    if look:
        relevant_ranks = []
        for item in look:
            if item in ranks:
                relevant_ranks.append([item, ranks[item]])
        relevant_ranks = quicksort_ranks(relevant_ranks)
        ordered_urls = list()
        for rank in relevant_ranks:
            ordered_urls.append(rank[0])
        return ordered_urls
    else:
        return None


def quicksort_ranks(ranks):
    if len(ranks) <= 1:
        return ranks
    else:
        pivot = ranks[0]
        less_than_pivot = list()
        equivalent_to_pivot = list()
        greater_than_pivot = list()
        for item in ranks:
            if item[1] < pivot[1]:
                less_than_pivot.append(item)
            elif item[1] == pivot[1]:
                equivalent_to_pivot.append(item)  # this will not need to be recursively sorted
            else:
                greater_than_pivot.append(item)
        # returning greater first because we want the best links first
        return quicksort_ranks(greater_than_pivot) + equivalent_to_pivot + quicksort_ranks(less_than_pivot)
